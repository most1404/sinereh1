...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _bootstrap() async {
    ...
    // هر بار برنامه باز می‌شود: اگر ساب‌اسکریپشنی ثبت شده، بروزرسانی کن؛
    // در هر صورت، سرورهای موجود را پینگ بگیر تا لیست خالی/بدون‌عدد نماند.
    if (_subscriptions.isNotEmpty) {
      unawaited(_refreshAllSubscriptions(silent: true).then((_) {
        if (_configs.isNotEmpty) {
          unawaited(_pingService.pingAll(_configs).then((_) {
            _storage.saveConfigs(_configs);
            if (mounted) setState(() {});
          }));
        }
      }));
  ...
  Future<void> _connectFastest() async {
    ...
    try {
      // یک تست سریع اولیه اگر هنوز هیچ‌کدام تست نشده‌اند
  ...
  Future<void> _failoverFromDeadServer(VpnConfig deadConfig) async {
    ...
    final pool = _activeConfigs.where((c) => c.id != deadConfig.id).toList();
    if (pool.isEmpty) return;

    await _pingService.pingAll(pool, concurrency: 3);
    await _storage.saveConfigs(_configs);
