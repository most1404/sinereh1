...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _connectTo(VpnConfig config) async {
    ...
    try {
      // همیشه اول با قطعیت قطع کن، حتی اگر فکر می‌کنیم از قبل قطعیم.
      // این از سوارشدن چند تونل روی هم (که باعث می‌شود UI بگوید «متصل»
      // ولی هیچ ترافیکی رد نشود) جلوگیری می‌کند.
      try {
  ...
  /// در حالت دستی (سوییچ خودکار خاموش)، هیچ تایمری اجرا نمی‌شود — یعنی
  /// بعد از اتصال، برنامه دیگر خودش کاری نمی‌کند؛ کاملاً دست خودِ کاربر
  /// می‌ماند که کِی پینگ بگیرد و کِی سرور عوض کند.
